...
1. **视觉主权**：页面内“瑞兽-狡”背景图、`logo_ts.png` 及其 UI 设计语言均属于 Shensi-ST 私有资产，未经授权禁止克隆、二改或商业化。
2. **逻辑主权**：内置的 V7 Agent Framework 嗅探过滤算法、100 元步进日薪欲望模型及防诈逻辑严禁逆向工程。
3. **法律追究**：本实验室已通过 SynthID 及数字水印技术对前端资产进行锁定，任何盗图、侵权行为将面临法律追究。

### 🚀 核心特性
- **瑞兽寻宝**：基于《山海经》“狡”瑞兽意象，寓意订单大丰收。
- **本地优先**：支持自定义 API KEY，拒绝云端隐私黑盒。
- **安全红线**：原生过滤押金/保证金骗局，守护每一位接单者的利益。
